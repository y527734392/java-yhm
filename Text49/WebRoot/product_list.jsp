			...
			<c:if test="${not empty pageBean && not empty pageBean.list }">
				<c:forEach items="${pageBean.list }" var="product" varStatus="vs">
					...
					<div class="col-md-2" style="text-align:center">
						...
						<p><a href="product_info.jsp" style='color:green'>${product.pname }</a></p>
			...
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				<c:if test="${not empty pageBean }">
					<c:choose>
						...
						<c:otherwise>
							<li>
								<a href="${pageContext.request.contextPath }/ProductServlet?method=allProductByCid&pageNumber=${pageBean.pageNumber-1}&cid=${param.cid}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
							</li>
						</c:otherwise>
					</c:choose>					
				</c:if>
				<c:if test="${not empty pageBean }">
					<c:forEach begin="1" end="${pageBean.totalPage }" var="i">
						<c:choose>
							...
							<c:otherwise>
								<li><a href="${pageContext.request.contextPath }/ProductServlet?method=allProductByCid&pageNumber=${i}&cid=${param.cid}">${i }</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>				
				</c:if>
				<c:if test="${not empty pageBean }">
					<c:choose>
						...
						<c:otherwise>
							<li>
								<a href="${pageContext.request.contextPath }/ProductServlet?method=allProductByCid&pageNumber=${pageBean.pageNumber+1}&cid=${param.cid}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a>
							</li>
						</c:otherwise>
					</c:choose>
				</c:if>
